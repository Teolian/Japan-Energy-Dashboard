...
export const useDemandStore = defineStore('demand', () => {
  ...
  const prevKansaiMetrics = ref<DemandMetrics | null>(null)
  ...
  async function fetchAllDemandData(date?: string) {
    ...
    // Fetch previous day data for trend comparison
    const prevDate = getPreviousDate(targetDate)
    try {
      const [prevTokyoData, prevKansaiData] = await Promise.all([
        getDemandData('tokyo', prevDate),
        getDemandData('kansai', prevDate)
      ])

      prevTokyoMetrics.value = calculateMetrics(prevTokyoData)
      prevKansaiMetrics.value = calculateMetrics(prevKansaiData)
    } catch (err) {
      // Previous day data might not exist, silently ignore
      prevTokyoMetrics.value = null
      prevKansaiMetrics.value = null
    }
  }
