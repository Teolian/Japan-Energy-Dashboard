...
export async function fetchSolarForecast(area: Area, date: string): Promise<SolarForecast> {
  // Try to fetch from static files first (generated by GitHub Actions)
  try {
    const fileUrl = `/data/jp/weather/weather-${area}-${date}.json`
    const fileResponse = await fetch(fileUrl)

    if (fileResponse.ok) {
      const forecast: SolarForecast = await fileResponse.json()
      console.log(`[WeatherClient] Loaded ${area} forecast from file: ${fileUrl}`)
      return forecast
    }
  } catch (fileError) {
    console.warn(`[WeatherClient] Failed to load from file, trying API:`, fileError)
  }
